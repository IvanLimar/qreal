...
void import()
{
	...
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.1:1/request");
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.1:1/run");
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.1:0/request");
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.1:0/run");
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.0:1/request");
	sysyem("echo 1 > /sys/class/pwm/ehrpwm.0:1/run");
	sysyem("echo 1 > /sys/class/pwm/ecap.0/request");
	sysyem("echo 1 > /sys/class/pwm/ecap.0/run");
	sysyem("echo 1 > /sys/class/pwm/ecap.1/request");
	sysyem("echo 1 > /sys/class/pwm/ecap.1/run");
	sysyem("echo 1 > /sys/class/pwm/ecap.2/request");
	sysyem("echo 1 > /sys/class/pwm/ecap.2/run");
	sysyem("#cat /sys/class/pwm/ecap_cap.0/config > /dev/null");
	sysyem("#cat /sys/class/pwm/ecap_cap.1/config > /dev/null");
	sysyem("#cat /sys/class/pwm/ecap_cap.2/config > /dev/null");
	sysyem("echo 1 > /sys/devices/virtual/input/input1/enable_device");
	sysyem("#amixer -q set PCM 127");
#endif
...
//        wprintf(L"pc=%i mem[pc]=%i x=%i mem[x]=%i rf=%f\n", pc, mem[pc], x, mem[x], rf);
		switch (mem[pc++])
		{
			...
			case SETMOTOR:
				...
				wprintf(L"i2cset -y 2 0x48 0x%x 0x%x w\n", 0x14 + n - 1, r);
				snprintf(i2ccommand, I2CBUFFERSIZE, "i2cset -y 2 0x48 0x%x 0x%x w", 0x14 + n - 1, r);
				system(i2ccommand);
				break;
#ifdef WITHDIGITALSENSORS
			case GETDIGSENSOR:
